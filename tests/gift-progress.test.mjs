...
const { formatGiftAmount, formatGoalAmount, formatGiftPercentage } = await loadPureModule('../src/lib/gifts/format.ts');
...
test('Progress joins by gift_id, preserves order and uses only RPC goal aggregates', () => {
  ...
  const rows = [
    ...
    { ...row, external_reference: 'private-reference', contributor_name: 'private-name',
      contributor_phone: 'private-phone', message: 'private-message', amount: 'private-amount',
      vest_name: 'private-vest', regional_division: 'private-regional' },
  ...
  for (const privateValue of ['private-reference', 'private-name', 'private-phone', 'private-message',
    'private-amount', 'private-vest', 'private-regional']) {
...
test('Missing goal rows become zero after a successful RPC response; unavailable or invalid data does not', () => {
  const expectedZero = { target_amount: 3500, total_raised: 0, percentage: 0, remaining_amount: 3500, goal_reached: false };
  assert.deepEqual(combineGiftProgress([gift], [])[0].progress, expectedZero);
  assert.deepEqual(combineGiftProgress([gift], [{ ...row, gift_id: 'another-gift' }])[0].progress, expectedZero);
...
test('pt-BR amounts use two decimal places and percentages stay visually clean', () => {
  ...
  assert.equal(money(600), 'R$ 600,00');
  assert.equal(money(1250), 'R$ 1.250,00');
  assert.equal(money(3500), 'R$ 3.500,00');
  assert.equal(money(12.5), 'R$ 12,50');
  assert.equal(formatGiftAmount(75).replace(/\u00a0/g, ' '), 'R$ 75,00');
