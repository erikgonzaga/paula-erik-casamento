...
export function Navigation() {
  ...
  return <header className="site-header"><a className="brand-link" href="#inicio" aria-label="Paula e Erik — início"><WeddingLogo size="navbar" /></a><button ref={button} className="menu-toggle" aria-expanded={open} aria-controls="navigation" onClick={() => setOpen(!open)}>{open ? 'Fechar ×' : 'Menu ☰'}</button><nav id="navigation" aria-label="Navegação principal" className={open ? 'navigation open' : 'navigation'}>{links.map(([label, href]) => <a key={href} href={href} onClick={() => setOpen(false)}>{label}</a>)}<a className="nav-rsvp" href="#presenca" onClick={() => setOpen(false)}>Confirme sua presença <span aria-hidden="true">↗</span></a></nav></header>;
}
