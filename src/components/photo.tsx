...
export function Photo({ caption, className = '', number = '01' }: Props) {
  ...
  return <figure className={`photo ${className}`}><div className="photo-surface"><DreamPhoto objectPosition={number === '01' ? '50% 45%' : '50% 50%'} src={photo.src} alt={photo.alt} fill sizes="(max-width: 600px) 88vw, 44vw" preload={number === '01'} /></div><figcaption><span>{number} / {caption}</span><ArrowUpRightIcon /></figcaption></figure>;
}
